...
EXP_NAME="BiRealNet10_FFF_teacher_Dis-729_student_Dis-726_batchnorm_freeze_False_manual_same_forward_distillation_head_LambdaLR_lr${LR}_CDC{$CDC}_RDC{$RDC}_CLC{$CLC}_RLC{$RLC}"
CAPTION='BiReal10_distillation'
python train.py --dataset coco --arch BiRealNet10 --lrScheduler LambdaLR --caption ${CAPTION} --coco_path /media/ramdisk/Datasets/coco --depth 10 --output_folder ${OUTPUT_FOLDER} --exp_name ${EXP_NAME} --lr ${LR} --batch_size 8 --epochs 12 --fdc ${FDC} --cdc ${CDC} --rdc ${RDC} --clc ${CLC} --rlc ${RLC} #--freeze_batchnorm
