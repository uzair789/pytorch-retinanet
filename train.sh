...
EXP_NAME="FP_distillation_teacher_Dis-716_resnet34_student_Dis-644_resnet18_oldScheduler"
#CAPTION='BiReal18_distillation'
CAPTION='FP_distillation'
python train.py --dataset coco --arch FP --lrScheduler OldScheduler --caption ${CAPTION} --coco_path /media/School/Datasets/coco --depth 18 --output_folder ${OUTPUT_FOLDER} --exp_name ${EXP_NAME} --lr ${LR} --batch_size 8 --epochs 12 --fdc ${FDC} --cdc ${CDC} --rdc ${RDC} --clc ${CLC} --rlc ${RLC} --warmup --normalization --change_teacher #--freeze_batchnorm
